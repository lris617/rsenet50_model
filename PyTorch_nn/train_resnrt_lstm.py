...
class LSTMImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(1, self.seq_len + 1):
            img_path = os.path.join(self.root_dir, sample_id, f"combined_{i}.png")
...
def main():
    ...
    # 数据路径
    train_img_dir = "PyTorch_nn/dataset4lstm14/train"
    train_label_csv = os.path.join(train_img_dir, "labels.csv")
    val_img_dir = "PyTorch_nn/dataset4lstm14/val"
    ...
    # 记录loss
    loss_record_path = f"report/cnnlstm14.2/cnnlstm_loss{cnn_type}_bs{batch_size}_lr{lr}_feature{feature_dim}_lstm_layers{lstm_layers}_hiddendim{lstm_hidden_dim}.csv"
    if not os.path.exists(loss_record_path):
        with open(loss_record_path, "w") as f:
            f.write("epoch,train_loss,val_loss,batch_size,lr\n")

    # 保存top3模型
    model_save_dir = f"report/cnnlstm14.2/{cnn_type}_bs{batch_size}_lr{lr}_feature{feature_dim}_lstm_layers{lstm_layers}_hiddendim{lstm_hidden_dim}"
