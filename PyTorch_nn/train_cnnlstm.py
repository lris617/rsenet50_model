...
def main():
    ...
    # 记录loss
    loss_record_path = f"report/cnnlstm_loss{cnn_type}_bs{batch_size}_lr{lr}_lstm_layers{lstm_layers}_hiddendim{lstm_hidden_dim}.csv"
    if not os.path.exists(loss_record_path):
        with open(loss_record_path, "w") as f:
            f.write("epoch,train_loss,val_loss,batch_size,lr\n")

    # 保存top3模型
    model_save_dir = f"report/cnnlstm14.1/{cnn_type}_bs{batch_size}_lr{lr}_lstm_layers{lstm_layers}_hiddendim{lstm_hidden_dim}"
